...
InputGeneratorManager *InputGeneratorManager::m_instance = NULL;

InputGeneratorManager::InputGeneratorManager(QObject *par) :
  QObject(par)
{
  refreshGenerators();
}

InputGeneratorManager::~InputGeneratorManager()
{
}

InputGeneratorManager &InputGeneratorManager::instance()
{
  if (!InputGeneratorManager::m_instance)
    InputGeneratorManager::m_instance = new InputGeneratorManager;
  return *InputGeneratorManager::m_instance;
}

QList<QAction *> InputGeneratorManager::createActions() const
{
...
BatchJob* InputGeneratorManager::performBatchCalculation(
    QWidget *windowParent, const QAction &action, const MongoModel &model)
{
  ...
  BatchJob *batch = new BatchJob(scriptFilePath, this);
  // Ensure that the batch pointer will be cleaned up if we return early:
  QScopedPointer<BatchJob> cleanup(batch);
  ...
  for (MolRefList::const_iterator it = moleculeRefs.begin(),
    ...
    if (AvogadroTools::createMolecule(*it, mol) && mol.atomCount() > 0) {
      qDebug() << "Submitting job for " << QString::fromStdString(mol.formula());
      if (batch->submitNextJob(mol))
        continue;
    }
    qWarning() << "Error submitting job.";
  }

  return cleanup.take();
}

void InputGeneratorManager::refreshGenerators()
{
  m_scriptFiles.clear();
...
bool InputGeneratorManager::queryProgramName(const QString &scriptFilePath,
                                             QString &name) const
{
...
QAction* InputGeneratorManager::createAction(const QString &label,
