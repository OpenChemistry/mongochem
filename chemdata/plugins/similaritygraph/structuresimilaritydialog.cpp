...
void StructureSimilarityDialog::setMolecules(const std::vector<ChemData::MoleculeRef> &molecules)
{
  ...
  for(size_t i = 0; i < m_molecules.size(); i++){
    const ChemData::MoleculeRef &refI = m_molecules[i];
    ...
    for(size_t j = i + 1; j < m_molecules.size(); j++){
      const ChemData::MoleculeRef &refJ = m_molecules[j];
